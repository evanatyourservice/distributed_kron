from typing import Any, List, Optional, Union, Callable, Tuple, TypeVar
...
def scale_by_quad(
    ...
    **kwargs,
    ...
    Author: https://github.com/evanatyourservice
        ...
        params_partition_specs: pytree same structure as params of jax.sharding.PartitionSpec.
        preconditioner_partition_spec: `None` or `PartitionSpec(str | None, str | None)`,
            ...
            from params_partition_specs that matches first preconditioner axis to
    ...
    def init_fn(params, return_partition_specs_only=False):
        ...
        if have_params_sharding:
            params_sharding_ = jax.tree.map(
                lambda p, sh: PartitionSpec(*(sh + (None,) * (len(p.shape) - len(sh)))), params, params_sharding_
            )
        preconditioner_sharding_ = preconditioner_partition_spec
        if preconditioner_partition_spec is not None:
            if len(preconditioner_partition_spec) < 2:
                preconditioner_sharding_ = PartitionSpec(preconditioner_partition_spec[0], None)

        # reshape params shaped () to (1,) to make things simpler
        params = jax.tree.map(lambda p: p[None] if len(p.shape) == 0 else p, params)
        if have_params_sharding:
            params_sharding_ = jax.tree.map(
                lambda sh: PartitionSpec(None) if sh == PartitionSpec() else sh, params_sharding_
            )
        ...
        dim_diag = jax.tree.map(
            lambda p, s: _get_preconditioner_types(p.shape[int(s) :], max_size_dense, max_skew_dense),
            params,
            scanned_layers_,
        )
        ...
        if have_params_sharding:
            scanned_dim_sharding = jax.tree.map(
                lambda sh, s: PartitionSpec(sh[0]) if s else None, params_sharding_, scanned_layers_
            )
            sharding_without_scan = jax.tree.map(
                lambda sh, s: PartitionSpec(*(sh[int(s) :])), params_sharding_, scanned_layers_
            )
        ...
        if partition_grads_into_blocks:
            partitioners = jax.tree.map(
                lambda _, ps, dd: BlockPartitioner(ps, block_size, dd), params, merged_shapes, dim_diag
            )
    ...
    def update_fn(updates: base.Updates, state: dict, params: base.Params = None):
        ...
        if have_flax:
            boxed_updates, grads_structure = jax.tree.flatten(
                updates, is_leaf=lambda g: isinstance(g, (chex.Array, nn.Partitioned, jax.ShapeDtypeStruct))
            )
        ...
        if partition_grads_into_blocks:
            ...
            momentum_updates = jax.tree.map(
                lambda _, g, s: _map_fn(False, 0, int(s), lambda x, bs=block_size: _pad_and_stack_matrices(x, bs), g),
                dummy_updates_tree,
                momentum_updates,
                scanned_layers_,
            )
        ...
        if have_qs_sharding:
            ...
            Qs_sharding = jax.tree.map(
                add_dims_to_spec, dummy_updates_tree, Qs_sharding_no_leading_dims, scanned_dim_sharding
            )
        ...
        if partition_grads_into_blocks:
            precond_gs = jax.tree.map(
                lambda g, s, ps: _map_fn(
                    False, 0, int(s), lambda p, shapes=ps: _unstack_and_unpad_matrices(p, shapes), g
                ),
                precond_gs,
                scanned_layers_,
                partitioned_shapes,
...
def _merge_small_dims(
    ...
    if np.all(np.array(shape_to_merge) == 1):  # handles shape (1,)
        return ([1], [True], PartitionSpec(None) if sharding_to_merge is not None else None)
    ...
    return (merged_shape, merged_diag, PartitionSpec(*merged_sharding) if sharding_to_merge else None)
